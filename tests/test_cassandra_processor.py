...
from scrapi.processing.cassandra import CassandraProcessor, DocumentModelV2, VersionModel
...
def test_process_raw():
    ...
    queryset = DocumentModelV2.objects(docID='someID', source=RAW['source'])
...
def test_process_normalized():
    ...
    queryset = DocumentModelV2.objects(docID=RAW['docID'], source=RAW['source'])
...
def test_versions():
    ...
    test_db.process_normalized(RAW, NORMALIZED)
    doc = DocumentModelV2.objects(docID=RAW['docID'], source=RAW['source'])[0]
